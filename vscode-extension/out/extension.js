...
class DebuggerViewProvider {
    _extensionUri;
    static viewType = 'software-debugger.view';
    _view;
    _workspaceSummary = "";
    _outputChannel;
    _lastResults = [];
    ...
    resolveWebviewView(webviewView, context, _token) {
        ...
        webviewView.webview.onDidReceiveMessage((data) => {
            if (data.type === 'analyze') {
                this.analyzeWorkspace(data.config);
            }
            else if (data.type === 'generateBlueprints') {
                this.generateIndependentDiagrams(data.config);
            }
            else if (data.type === 'chat') {
                this.chatWithRepo(data.message);
            }
        });
    }
    analyzeWorkspace(config) {
        if (!this._view)
            return;
        ...
        this.runPythonProcess(args, (res) => {
            ...
            this._workspaceSummary = res.map((r) => `### ${r.name}\n${r.content}`).join("\n\n").substring(0, 20000);
            this._view?.webview.postMessage({ type: 'analysisResults' });
            ResultPanel.createOrShow(res, false);
        });
    }
    generateIndependentDiagrams(config) {
        if (!this._view || !this._workspaceSummary)
            return;
    ...
    runPythonProcess(args, onSuccess) {
        const proc = (0, child_process_1.spawn)('python', [path.join(this._extensionUri.fsPath, '..', 'extension_api.py'), ...args]);
        let out = '';
        ...
        proc.stderr.on('data', (d) => {
            ...
            if (line.includes('---'))
                this._view?.webview.postMessage({ type: 'progress', message: line.replace(/---/g, '').trim() });
        });
        proc.on('close', (c) => {
            ...
            if (match && match[1]) {
                ...
                if (c !== 0)
                    this._view?.webview.postMessage({ type: 'error', message: 'Engine failed.' });
                else
                    this._view?.webview.postMessage({ type: 'error', message: 'Process finished but no results were returned.' });
            }
        });
    }
    chatWithRepo(q) {
        const proc = (0, child_process_1.spawn)('python', [path.join(this._extensionUri.fsPath, '..', 'extension_api.py'), 'chat', '', q, this._workspaceSummary]);
        let out = '';
        proc.stdout.on('data', (d) => { out += d.toString(); });
        proc.on('close', () => {
            const match = out.match(/==DEBUGGER_RESULT_START==([\s\S]*?)==DEBUGGER_RESULT_END==/);
            if (match && match[1])
                this._view?.webview.postMessage({ type: 'chatResponse', data: JSON.parse(match[1].trim()) });
        });
    }
}
class ResultPanel {
    ...
    constructor(panel, results) {
        ...
        this._panel.webview.onDidReceiveMessage(m => {
            if (m.type === 'viewReady')
                this._panel.webview.postMessage({ type: 'renderAnalysis', data: this._results, isAppend: false });
        });
    }
    static createOrShow(results, isAppend) {
